...
def buscar_arquivos_json(caminho_base):
    """
    Busca recursivamente por todos os arquivos JSON em um diretório.
    """
    arquivos_json = []
    ...
    try:
        ...
        print(f"Erro na busca de arquivos JSON: {e}")
    
    return arquivos_json

def obter_campos_unicos(arquivos_json, tipo_conversao):
    """
    Obtém uma lista de todos os campos únicos encontrados nos arquivos JSON,
    adaptado para cada tipo de conversão.
...
def busca_abrangente(query, arquivos_json, tipo_conversao, filtro_campos=None):
    """
    Realiza uma busca abrangente em todos os campos dos mapeamentos de múltiplos arquivos.
    Adaptado para suportar tanto OC3_PARA_DATAMESH quanto SAC_PARA_OC3.
    """
    resultados = []
...
def show_campo_search():
    ...
    # --- Início do conteúdo da função original ---
    ...
    # Definir caminho base para busca de arquivos
    caminho_base = "mapeamentos-de-para"
    
    # Determinar subpasta de acordo com o tipo de conversão
    if tipo_conversao == "OC3_PARA_DATAMESH":
        caminho_mapeamentos = os.path.join(caminho_base, "mapeamentos-oc3-datamesh")
        sistema_origem = "OC3"
        sistema_destino = "DataMesh"
    else:  # SAC_PARA_OC3
        caminho_mapeamentos = os.path.join(caminho_base, "mapeamentos-sac-oc3")
        sistema_origem = "SAC"
        sistema_destino = "OC3"
    
    st.write(f"**Caminho dos mapeamentos**: {caminho_mapeamentos}")
    ...
    with st.expander("ℹ️ Ajuda - Como usar a busca de campos"):
        ...
        #### Exibir campos
        - **Campos comuns**: Mostra apenas os campos mais relevantes para o modo selecionado
        - **Todos os campos disponíveis**: Mostra todos os campos encontrados nos mapeamentos
    ...
    with st.spinner(f"Carregando arquivos JSON de {caminho_mapeamentos}..."):
        arquivos_json = buscar_arquivos_json(caminho_mapeamentos)
    ...
    campos_unicos = obter_campos_unicos(arquivos_json, tipo_conversao)
